$(function() {
  ...
  function finishRegionResize(event) {
    ...
    if (this.width() < 100) {
      ...
      console.log(this.closest('.row').prev('.row').find('.region').css('width', 960));
  ...
  function createRegion(event, $context) {
    ...
    // newRegions with no name should not be created.
    if (!newRegion.find('p').text()) {
      return false;
      console.log('A nameless newRegion has appeared!');
    }
    ...
    $($context).find('.region').removeClass('empty');

    $($context).find('.splitter').unbind('mousedown.regionCreate');
  }

  function placeRegion(event, $context) {
    // this = .row
  ...
  $('.splitter').live('mousedown', function(event) {
    ...
    if ($(this).hasClass('splitter-edge')) {
      fn = $.proxy(placeRegion, $row);
      $(document).bind('mouseup.regionPlace', data, fn);
    }

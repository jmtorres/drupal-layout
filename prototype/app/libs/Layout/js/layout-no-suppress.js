$(function() {
  ...
  function depositRegion(event, $context) {
    ...
    console.log($nextRow);
    console.log(this.next('.row').html());

    var $deposit = this.find('.new');
    $deposit.removeClass('new');

    $withdrawal = $nextRow.find('.region').detach();
    console.log(this.find('.splitter'));
    
    // Add class "splitter-edge" to splitters on row edges (not working yet).
    if (event.side == 'left' && $($context).prev().length === 0) {
      this.find('.splitter-left').addClass('splitter-edge');
    }
    if (event.side == 'right' && $($context).next().length === 0) {
      this.find('.splitter-right').addClass('splitter-edge');
    }
    if ($($context).prev().length !== 0 || $($context).next().length !== 0) {
      this.find('.splitter').removeClass('splitter-edge');
    }
    console.log(this.find('.splitter'));
